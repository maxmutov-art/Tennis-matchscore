...
async function exportToPDF() {
    ...
    const title = "ATP-Style Tennis Match Report";
    ...
    // --- Render SCOREBOARD ---
    const scoreCanvas = await html2canvas(scoreboard, {
        scale: 2,
        backgroundColor: "#0b1220"
    });

    const scoreImg = scoreCanvas.toDataURL("image/png");
    pdf.addImage(scoreImg, "PNG", 10, 18, 190, 40);
    ...
    pdf.addImage(statsImg, "PNG", 10, 62, 190, 50);

    // --- Render MOMENTUM GRAPH (REAL CANVAS) ---
    const momentumCanvas = document.getElementById("momentumCanvas");
    const momentumImg = momentumCanvas.toDataURL("image/png", 1.0);

    pdf.addImage(momentumImg, "PNG", 10, 118, 190, 70);

    // --- Footer ---
    pdf.setFontSize(10);
    pdf.text("Generated by ATP-Style Tennis Scoreboard", 105, 195, { align: "center" });

    const fileName =
        `${playerA.name}_vs_${playerB.name}_match_report.pdf`
            .replace(/\s+/g, "_");
